...
def compute_3d_translation(delta_x, delta_y, Z, K):
    ...
    参数:
    delta_x, delta_y: 像素平移量
    Z: 目标距离相机的深度（单位：米）
    K: 相机内参矩阵 (3x3)，包含焦距和主点

    返回:
    delta_X, delta_Y: 目标在相机坐标系下的三维平移量
    """
    fx, fy = K[0, 0], K[1, 1]  # 焦距
    cx, cy = K[0, 2], K[1, 2]  # 主点坐标
    
    # 计算三维平移
    delta_X = (delta_x * Z) / fx  # 在相机坐标系下的x方向平移
    delta_Y = (delta_y * Z) / fy  # 在相机坐标系下的y方向平移
        ...
        if contours:
            ...
            if area > 100:  
                ...
                if 0.5 < circularity < 1.3:  
                    ...
                    # 构建矩形的四个角点 (左下、右下、右上、左上)
                    lb = (int(center_x - radius), int(center_y + radius))
                    rb = (int(center_x + radius), int(center_y + radius))
                    rt = (int(center_x + radius), int(center_y - radius))
                    lt = (int(center_x - radius), int(center_y - radius))

                    corners = np.array([lb, rb, rt, lt])

                    self.get_logger().info(f'Tracking ball at ({center_x}, {center_y}), area={area:.1f}, circularity={circularity:.2f}')
